...
public class DeviceGenericParams : IXdrCodec
{
    ...
    public DeviceGenericParams( XdrDecodingStreamBase decoder ) : this()
    {
        this.Decode( decoder );
    }

    /// <summary>   Decodes an instance of a <see cref="DeviceGenericParams"/>. </summary>
    /// <param name="decoder">  XDR stream from which decoded information is retrieved. </param>
    /// <returns>   The <see cref="DeviceGenericParams"/>. </returns>
    public static DeviceGenericParams DecodeInstance( XdrDecodingStreamBase decoder )
    {
        return new DeviceGenericParams( decoder );
    }
